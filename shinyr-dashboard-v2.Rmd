...
theta <- .2 #prob that handwashing has worn off after one hour
...
T_end <- 10000 #run model for 1000 time steps
...
ui <- fluidPage(theme = shinytheme("lumen"),
                ...
                sidebarLayout(
                    sidebarPanel(
                        ...
                        sliderInput(inputId = "theta", label = "theta", min = 0, max = 1, value = .9, step = .1, round = -1, ticks = TRUE),
...
server <- function(input, output) {
    ...
    analysis <- reactive({
        ...
        c(max(data$I)*.2, sum(data$I*.01)/24)
    })
    ...
    output$hospitalizations <- renderText({
        paste("Peak hospitalizations:", floor(analysis()[[1]]))
    })
    
     output$deaths <- renderText({
        paste("Cumulative deaths:", floor(analysis()[[2]]))
    })
}
